...
class AuditServiceTest {
    ...
    @Test
    @DisplayName("Get All Audit Logs - Returns records ordered by latest")
    void testGetAllAuditLogs() {
        when(repository.findAllByOrderByIdDesc()).thenReturn(List.of(testLog));
        ...
        assertEquals("FLAG_CREATED", results.get(0).getEventType());
    }

    @Test
    @DisplayName("Get Audit Logs By Flag Key - Returns records for key")
    void testGetAuditLogsByFlagKey() {
        when(repository.findByFlagKeyOrderByTimestampDesc("NEW_CHECKOUT")).thenReturn(List.of(testLog));

        List<AuditLog> results = auditService.getAuditLogsByFlagKey("NEW_CHECKOUT");
    ...
    void testGetAuditLogById_Success() {
        ...
        AuditLog found = auditService.getAuditLogById(1L);
    ...
    @Test
    @DisplayName("Get Audit Log By ID - Not Found Throws RuntimeException")
    ...
    @Test
    @DisplayName("Consume Event - Persists AuditLog in repository")
    void testConsumeEvent() {
        AuditEventConsumer consumer = new AuditEventConsumer(repository);
        FlagEvent event = new FlagEvent("FLAG_UPDATED", "DARK_MODE", LocalDateTime.now().toString());
